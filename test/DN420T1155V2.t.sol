...
contract CastTokenV1WithHooks is DN420Mock {
    uint256 public beforeCounter;
    uint256 public afterCounter;

    constructor() DN420Mock(
        "Token", "TKN", "BASE_URL", 18, 10 ** 5
    ) {}
...
contract ERC1155HooksTest is SoladyTest, ERC1155TokenReceiver {
    ...
    function _checkCounters() internal view {
        require(
            expectedBeforeCounter == CastTokenV1WithHooks(msg.sender).beforeCounter(),
            "Before counter mismatch."
        );
        require(
            expectedAfterCounter == CastTokenV1WithHooks(msg.sender).afterCounter(),
    ...
    // function _testHooks(CastTokenV1WithHooks token) internal {
    ...
    //     CastTokenV1WithHooks token = new CastTokenV1WithHooks();
...
contract DN420T1155V2Test is SoladyTest, ERC1155TokenReceiver {
    DN420Mock token;
    ...
    function testAuthorizedEquivalence(address by, address from, bool isApprovedAccount) public pure {
        bool a = true;
        bool b = true;
        /// @solidity memory-safe-assembly
        assembly {
    ...
    function testMintToEOA(uint256) public {
        ...
        assertEq(token.nftBalanceOf(t.to, t.id), t.mintAmount);
    }
    ...
    function testMintToERC1155Recipient(uint256) public {
        ...
        console.log("t.mintAmount: ", t.mintAmount);
        token.mint(address(to), t.id, t.mintAmount, t.mintData);

        assertEq(token.nftBalanceOf(address(to), t.id), t.mintAmount);
    ...
    function testBatchMintToEOA(uint256) public {
        ...
        for (uint256 i = 0; i != t.n; i++) {
            ...
            assertEq(token.nftBalanceOf(t.to, id), userMintAmounts[t.to][id]);
        }
    }
    ...
    //     if (_random() % 2 == 0) {
    //         vm.startPrank(t.to);
    //         token.burn( t.id, t.burnAmount);
    //         vm.stopPrank();
    //         return;
    //     } else {
    //         vm.startPrank(t.to);
    //         // _setApprovalForAll(address(this), true);
    //         _setApprovalForAll(t.to, true);
    ...
    //     assertEq(token.nftBalanceOf(t.to, t.id), t.mintAmount - t.burnAmount);
    // }
    ...
    //     if (_random() % 2 == 0) {
    //         vm.startPrank(t.to);
    //         token.batchBurn(t.ids, t.burnAmounts);
    //         vm.stopPrank();
    //         return;
    //     } else {
    //         _setApprovalForAll(address(this), true);
    ...
    function testSafeTransferFromToEOA(uint256) public {
        ...
        if (t.to == t.from) {
            ...
            assertEq(token.nftBalanceOf(t.to, t.id), t.transferAmount);
            assertEq(token.nftBalanceOf(t.from, t.id), t.mintAmount - t.transferAmount);
        }
    }
    ...
    function testSafeTransferFromToERC1155Recipient(uint256) public {
        ...
        assertEq(token.nftBalanceOf(address(to), t.id), t.transferAmount);
        assertEq(token.nftBalanceOf(t.from, t.id), t.mintAmount - t.transferAmount);
    }
    ...
    function testSafeTransferFromSelf(uint256) public {
        ...
        assertEq(token.nftBalanceOf(t.to, t.id), t.transferAmount);
        assertEq(token.nftBalanceOf(address(this), t.id), t.mintAmount - t.transferAmount);
    }
    ...
    function testSafeBatchTransferFromToEOA(uint256) public {
        ...
        for (uint256 i = 0; i != t.n; i++) {
            ...
            assertEq(token.nftBalanceOf(t.to, id), userTransferOrBurnAmounts[t.from][id]);
            assertEq(
                token.nftBalanceOf(t.from, id),
    ...
    function testSafeBatchTransferFromToERC1155Recipient(uint256) public {
        ...
        for (uint256 i = 0; i != t.n; i++) {
            ...
            assertEq(token.nftBalanceOf(t.from, id), userMintAmounts[t.from][id] - transferAmount);
        }
    }
    ...
    function testBatchBalanceOf(uint256) public {
        ...
        for (uint256 i = 0; i != t.n; i++) {
            assertEq(balances[i], token.nftBalanceOf(tos[i], t.ids[i]));
        }
    }
