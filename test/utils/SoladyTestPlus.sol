...
contract SoladyTestPlus is Brutalizer {
    ...
    function _random() internal returns (uint256 r) {
        ...
        assembly {
            ...
            for {} 1 {} {
             ...
             // 定义一个 10^25 的常数
            let adjustment := 10000000000000000000000000
            // 定义最大值为 2^256 / 10^25
            let maxVal := div(not(0), adjustment)

            // 进行模运算，确保 r 在 [1, maxVal - 1] 范围内
            r := add(mod(r, maxVal), 1)
        }
    }
